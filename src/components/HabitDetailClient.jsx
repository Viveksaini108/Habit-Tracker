  ...
  return (
    <div className="overflow-x-auto">
      <div className="flex min-w-[560px] gap-[3px]">
        <div className="mr-1 flex flex-col justify-between py-0.5 text-[9px] font-semibold uppercase text-faint">
            ...
            {week.map((key) => {
              ...
              return (
                <span
                  ...
                  className="h-[13px] w-[13px] rounded-[3px] transition hover:ring-1 hover:ring-faint"
                  style={{
                    backgroundColor: !active || isFuture ? 'rgb(var(--soft))' : done ? habit.color : 'rgb(var(--soft2))',
      ...
      <div className="mt-3 flex items-center justify-end gap-1.5 text-[10px] font-medium text-faint">
        Less
        <span className="h-[10px] w-[10px] rounded-[3px] bg-soft2" />
          ...
          <button
            ...
            className={`flex flex-col items-center gap-1 rounded-xl border px-1 py-2 text-center transition-all ${
                ...
                : active
                  ...
                  : 'cursor-not-allowed border-line-soft bg-soft text-faint'
            }`}
    ...
    <div>
      <div className="mb-3">
        ...
        <div className="mt-2 flex items-center justify-between">
          <span className="tnum text-[11px] text-faint">{draft.length}/500</span>
      ...
      {notes.length === 0 ? (
        ...
        <div className="space-y-2.5">
          {notes.map((n) => (
            <div key={n.id} className={`group rounded-xl border border-line-soft bg-soft/50 p-3 ${n.pending ? 'opacity-60' : ''}`}>
              <p className="whitespace-pre-wrap text-sm text-ink2">{n.body}</p>
              <div className="mt-1.5 flex items-center justify-between">
                <span className="text-[11px] text-faint">{n.created_at?.slice(0, 16) ?? ''}</span>
                <button
                  onClick={() => remove(n)}
                  className="rounded-lg p-1 text-faint opacity-0 transition group-hover:opacity-100 hover:bg-rose-50 hover:text-rose-500"
    ...
    <div>
      <div className="card p-5 sm:p-6">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div className="min-w-0">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <h1 className="text-xl font-extrabold tracking-tight text-ink">{habit.name}</h1>
            ...
            {habit.description ? <p className="mt-2 max-w-xl text-sm text-muted">{habit.description}</p> : null}
            <div className="mt-3 flex flex-wrap items-center gap-4 text-xs font-medium text-faint">
        ...
        <div className="mt-5 grid grid-cols-2 gap-3 sm:grid-cols-4">
          {[
            ...
            <div key={stat.label} className="rounded-xl bg-soft p-3">
              <p className="flex items-center gap-1.5 text-[11px] font-bold uppercase tracking-wide text-faint">
                <span className="text-accent">{stat.icon}</span>
                {stat.label}
              </p>
              <p className="tnum mt-1 text-lg font-extrabold text-ink">{stat.value}</p>
            </div>
          ))}
        </div>
        ...
        <div className="mt-4 flex items-center gap-2">
          ...
          <span className="tnum text-xs font-bold text-muted">{Math.round(s.weekRate * 100)}% this week</span>
        </div>
      </div>
      ...
      <div className="mt-4 grid grid-cols-1 gap-4 lg:grid-cols-3">
        <div className="space-y-4 lg:col-span-2">
          <div className="card p-5">
            <h3 className="mb-4 text-sm font-bold text-ink">Last 6 months</h3>
            <HabitHeatmap habit={habit} entries={entries} today={today} />
          </div>
          <div className="card p-5">
            <h3 className="mb-4 text-sm font-bold text-ink">Quick check-in — last 14 days</h3>
        ...
        <div className="card h-fit p-5">
          <h3 className="mb-4 text-sm font-bold text-ink">Progress notes</h3>
      ...
      <Modal open={deleteOpen} onClose={() => setDeleteOpen(false)} title="Delete habit?">
        <p className="text-sm text-ink2">
          <strong className="text-ink">{habit.name}</strong> and all its check-ins and notes will be permanently
