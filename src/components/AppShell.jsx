...
function SidebarContent({ user, pathname, onNavigate, onLogout, loggingOut }) {
  return (
    <div className="flex h-full flex-col">
      ...
      <nav className="flex-1 space-y-1 px-3">
        {NAV.map((item) => {
          ...
          return (
            ...
            >
              <Icon className={`h-[18px] w-[18px] ${active ? 'text-white' : 'text-slate-500 group-hover:text-white'}`} />
              {item.label}
            </Link>
          );
        })}
      ...
      <div className="border-t border-white/10 p-4">
        <div className="flex items-center gap-3">
          <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-brand-400 to-violet-500 text-sm font-bold text-white">
  ...
  return (
    <div className="min-h-screen lg:pl-64">
      ...
      <aside className="fixed inset-y-0 left-0 z-30 hidden w-64 bg-slate-950 lg:block">
        <SidebarContent user={user} pathname={pathname} onLogout={logout} loggingOut={loggingOut} />
      </aside>

      {/* Mobile top bar */}
      <header className="sticky top-0 z-20 flex h-14 items-center gap-3 border-b border-slate-200 bg-slate-950 px-4 lg:hidden">
        <button
      ...
      {mobileOpen ? (
        <div className="fixed inset-0 z-40 lg:hidden">
          <div
            ...
            aria-hidden="true"
          />
          <div className="absolute inset-y-0 left-0 w-72 animate-fade-up bg-slate-950 shadow-pop">
            <button
              onClick={() => setMobileOpen(false)}
